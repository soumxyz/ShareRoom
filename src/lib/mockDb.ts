import { generateRoomCode } from './fingerprint';
...
class MockDb {
    ...
    // Room Methods
    async createRoom(name: string, host_fingerprint: string): Promise<Room> {
    ...
    // Participant Methods
    async joinRoom(roomId: string, username: string, fingerprint: string): Promise<Participant> {
        const participants = this.getItems<Participant>('participants');
        let participant = participants.find(p => p.room_id === roomId && p.fingerprint === fingerprint);
